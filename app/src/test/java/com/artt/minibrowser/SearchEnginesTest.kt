...
class SearchEnginesTest {
    ...
    @Test fun translateUriDropsConflictingControlQuery() {
        assertEquals(
            ...
            buildTranslateUri(
                "https://example.com/a?keep=1&_x_tr_tl=fr&%5Fx_tr_hl=fr&_X_TR_SL=de#section",
                "ru",
            ),
        )
    }
    @Test fun translateUriDropsQueryContainingOnlyControlParams() {
        assertEquals(
            "https://example-com.translate.goog/a?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en",
            buildTranslateUri("https://example.com/a?_x_tr_tl=fr&_x_tr_hl=fr", "en"),
        )
    }
