...
class TabPreviewStoreHostTest {
    @Test
    fun newEngineViewHostReleasesRemovedTabIds() {
        val instrumentation = InstrumentationRegistry.getInstrumentation()
        val context = instrumentation.targetContext

        instrumentation.runOnMainSync {
            val previewStore = TabPreviewStore()
            val firstHost = GeckoEngineView(context)
            ...
            // Activity/AndroidView recreation creates a new EngineView. Old capture callbacks are
            // invalidated by the host generation change, so a legitimately reused tab id is safe.
            val secondHost = GeckoEngineView(context)
